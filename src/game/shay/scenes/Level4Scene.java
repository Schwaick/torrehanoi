...
public class Level4Scene extends BaseScene {

	private final static int necessaryMoves = 63;
	private int contador = 0;
	private Ring ring;
	private boolean ring1Touched=false,ring2Touched=false,ring3Touched=false,ring4Touched=false,ring5Touched=false,ring6Touched=false;
	...
	private Ring ring1, ring2, ring3, ring4, ring5, ring6;
	...
	private void createHUD()
	{
		...
		level = new Text(528, 385, resourcesManager.font, "Level: 0123456789", new TextOptions(HorizontalAlign.LEFT), vbom);
		...
		tower1 = new Sprite(150, 205, resourcesManager.tower_region, vbom) {
			public boolean onAreaTouched(TouchEvent pSceneTouchEvent, float pTouchAreaLocalX, float pTouchAreaLocalY) {
				...
				if(pSceneTouchEvent.getAction() == TouchEvent.ACTION_UP) {
					ring1Touched = ring2Touched = ring3Touched = ring4Touched = ring5Touched = ring6Touched = false;
	...
	private void selectRing(Ring currentRing) {
		if(!ring1Touched && !ring2Touched && !ring3Touched && !ring4Touched && !ring5Touched && !ring6Touched) {
	...
	private void changeRingPosition(Sprite tower, Ring ring) {
	    ...
	    if(stack3.size() == 6) {
	    	...
	    	pauseGame();
	    	levelCompleteWindow.display(4, contador, getMoves(), necessaryMoves);
	    }
	}
	...
	private void checkMoves() {
		DBHelper dbHelper = new DBHelper(activity);
		if(contador < dbHelper.getRecord(4)) {
			dbHelper.setRecord(4, contador);
		}
	}
	
	private int getMoves() {
		DBHelper dbHelper = new DBHelper(activity);
		return dbHelper.getRecord(4);
	}
