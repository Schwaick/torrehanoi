...
public class Level1Scene extends BaseScene {
	
	private static final int necessaryMoves = 7;
	private int contador = 0;
	private Ring ring;
	private boolean ring1Touched=false,ring2Touched=false,ring3Touched=false;
	...
	private void createHUD()
	{
		...
		level = new Text(540, 385, resourcesManager.font, "Level: 0123456789", new TextOptions(HorizontalAlign.LEFT), vbom);
		...
		tower1 = new Sprite(150, 205, resourcesManager.tower_region, vbom) {
			public boolean onAreaTouched(TouchEvent pSceneTouchEvent, float pTouchAreaLocalX, float pTouchAreaLocalY) {
				...
				if(pSceneTouchEvent.getAction() == TouchEvent.ACTION_UP) {
					ring1Touched = ring2Touched = ring3Touched = false;
	...
	private void selectRing(Ring currentRing) {
		if(!ring1Touched && !ring2Touched && !ring3Touched) {
	...
	private void changeRingPosition(Sprite tower, Ring ring) {
	    ...
	    if(stack3.size() == 3) {
	    	...
	    	pauseGame();
	    	levelCompleteWindow.display(1, contador, getMoves(), necessaryMoves);
	    }
	}
	...
	private void checkMoves() {
		DBHelper dbHelper = new DBHelper(activity);
		if(contador < dbHelper.getRecord(1)) {
			dbHelper.setRecord(1, contador);
		}
	}
	
	private int getMoves() {
		DBHelper dbHelper = new DBHelper(activity);
		return dbHelper.getRecord(1);
	}
