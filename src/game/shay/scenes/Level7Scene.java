...
public class Level7Scene extends BaseScene {

	private final static int necessaryMoves = 511;
	private int contador = 0;
	private Ring ring;
	private boolean ring1Touched=false,ring2Touched=false,ring3Touched=false,ring4Touched=false,ring5Touched=false,ring6Touched=false,ring7Touched=false,ring8Touched=false,ring9Touched=false;
	...
	private Ring ring1, ring2, ring3, ring4, ring5, ring6, ring7, ring8, ring9;
	...
	private void createHUD()
	{
		...
		level = new Text(535, 385, resourcesManager.font, "Level: 0123456789", new TextOptions(HorizontalAlign.LEFT), vbom);
		...
		tower1 = new Sprite(150, 205, resourcesManager.tower_region, vbom) {
			public boolean onAreaTouched(TouchEvent pSceneTouchEvent, float pTouchAreaLocalX, float pTouchAreaLocalY) {
				...
				if(pSceneTouchEvent.getAction() == TouchEvent.ACTION_UP) {
					ring1Touched = ring2Touched = ring3Touched = ring4Touched = ring5Touched = ring6Touched = ring7Touched = ring8Touched = ring9Touched = false;
	...
	private void selectRing(Ring currentRing) {
		if(!ring1Touched && !ring2Touched && !ring3Touched && !ring4Touched && !ring5Touched && !ring6Touched && !ring7Touched && !ring8Touched && !ring9Touched) {
	...
	private void changeRingPosition(Sprite tower, Ring ring) {
	    ...
	    if(stack3.size() == 9) {
	    	...
	    	pauseGame();
	    	levelCompleteWindow.display(7, contador, getMoves(), necessaryMoves);
	    }
	}
	...
	private void checkMoves() {
		DBHelper dbHelper = new DBHelper(activity);
		if(contador < dbHelper.getRecord(7)) {
			dbHelper.setRecord(7, contador);
		}
	}
	
	private int getMoves() {
		DBHelper dbHelper = new DBHelper(activity);
		return dbHelper.getRecord(7);
	}
