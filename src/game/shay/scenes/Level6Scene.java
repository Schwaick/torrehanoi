...
public class Level6Scene extends BaseScene {

	private final static int necessaryMoves = 255;
	private int contador = 0;
	private Ring ring;
	private boolean ring1Touched=false,ring2Touched=false,ring3Touched=false,ring4Touched=false,ring5Touched=false,ring6Touched=false,ring7Touched=false,ring8Touched=false;
	...
	private Ring ring1, ring2, ring3, ring4, ring5, ring6, ring7, ring8;
		...
		tower1 = new Sprite(150, 205, resourcesManager.tower_region, vbom) {
			public boolean onAreaTouched(TouchEvent pSceneTouchEvent, float pTouchAreaLocalX, float pTouchAreaLocalY) {
				...
				if(pSceneTouchEvent.getAction() == TouchEvent.ACTION_UP) {
					ring1Touched = ring2Touched = ring3Touched = ring4Touched = ring5Touched = ring6Touched = ring7Touched = ring8Touched = false;
	...
	private void selectRing(Ring currentRing) {
		if(!ring1Touched && !ring2Touched && !ring3Touched && !ring4Touched && !ring5Touched && !ring6Touched && !ring7Touched && !ring8Touched) {
	...
	private void changeRingPosition(Sprite tower, Ring ring) {
	    ...
	    if(stack3.size() == 8) {
	    	...
	    	pauseGame();
	    	levelCompleteWindow.display(6, contador, getMoves(), necessaryMoves);
	    }
	}
	...
	private void checkMoves() {
		DBHelper dbHelper = new DBHelper(activity);
		if(contador < dbHelper.getRecord(6)) {
			dbHelper.setRecord(6, contador);
		}
	}
	
	private int getMoves() {
		DBHelper dbHelper = new DBHelper(activity);
		return dbHelper.getRecord(6);
	}
