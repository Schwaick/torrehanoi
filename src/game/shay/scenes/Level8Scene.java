...
public class Level8Scene extends BaseScene {

	private final static int necessaryMoves = 1023;
	private int contador = 0;
	private Ring ring;
	private boolean ring1Touched=false,ring2Touched=false,ring3Touched=false,ring4Touched=false,ring5Touched=false,ring6Touched=false,ring7Touched=false,ring8Touched=false,ring9Touched=false,ring10Touched=false;
	...
	private Ring ring1, ring2, ring3, ring4, ring5, ring6, ring7, ring8, ring9, ring10;
	...
	private void createHUD()
	{
		...
		level = new Text(552, 385, resourcesManager.font, "Level: 0123456789", new TextOptions(HorizontalAlign.LEFT), vbom);
		...
		tower1 = new Sprite(150, 205, resourcesManager.tower_region, vbom) {
			public boolean onAreaTouched(TouchEvent pSceneTouchEvent, float pTouchAreaLocalX, float pTouchAreaLocalY) {
				...
				if(pSceneTouchEvent.getAction() == TouchEvent.ACTION_UP) {
					ring1Touched = ring2Touched = ring3Touched = ring4Touched = ring5Touched = ring6Touched = ring7Touched = ring8Touched = ring9Touched = ring10Touched = false;
	...
	private void createRings() {
		...
		ring6 = new Ring(5, 150, 215, resourcesManager.ring6_region, vbom) {
			@Override
		    public boolean onAreaTouched(TouchEvent pSceneTouchEvent, float pTouchAreaLocalX, float pTouchAreaLocalY) {
		        if (((Ring) this.getStack().peek()).getWeight() != this.getWeight())
		            return false;
		...
		ring7 = new Ring(4, 150, 245.5f, resourcesManager.ring7_region, vbom) {
			@Override
		    public boolean onAreaTouched(TouchEvent pSceneTouchEvent, float pTouchAreaLocalX, float pTouchAreaLocalY) {
		        if (((Ring) this.getStack().peek()).getWeight() != this.getWeight())
		            return false;
		...
		ring8 = new Ring(3, 150, 275.5f, resourcesManager.ring8_region, vbom) {
			@Override
		    public boolean onAreaTouched(TouchEvent pSceneTouchEvent, float pTouchAreaLocalX, float pTouchAreaLocalY) {
		        if (((Ring) this.getStack().peek()).getWeight() != this.getWeight())
		            return false;
		...
		ring9 = new Ring(2, 150, 305.5f, resourcesManager.ring9_region, vbom) {
			@Override
		    public boolean onAreaTouched(TouchEvent pSceneTouchEvent, float pTouchAreaLocalX, float pTouchAreaLocalY) {
		        if (((Ring) this.getStack().peek()).getWeight() != this.getWeight())
		            return false;
		...
		ring10 = new Ring(1, 150, 336, resourcesManager.ring10_region, vbom) {
			@Override
		    public boolean onAreaTouched(TouchEvent pSceneTouchEvent, float pTouchAreaLocalX, float pTouchAreaLocalY) {
		        if (((Ring) this.getStack().peek()).getWeight() != this.getWeight())
		            return false;
		...
		ring6.setStack(stack1);
		ring7.setStack(stack1);
		ring8.setStack(stack1);
		ring9.setStack(stack1);
		ring10.setStack(stack1);
		...
		ring6.setTower(tower1);
		ring7.setTower(tower1);
		ring8.setTower(tower1);
		ring9.setTower(tower1);
		ring10.setTower(tower1);
		...
		registerTouchArea(ring6);
		registerTouchArea(ring7);
		registerTouchArea(ring8);
		registerTouchArea(ring9);
		registerTouchArea(ring10);
		setTouchAreaBindingOnActionDownEnabled(true);

	}
	
	private void selectRing(Ring currentRing) {
		if(!ring1Touched && !ring2Touched && !ring3Touched && !ring4Touched && !ring5Touched && !ring6Touched && !ring7Touched && !ring8Touched && !ring9Touched && !ring10Touched) {
			...
			if(currentRing == ring6) ring6Touched = true;
			if(currentRing == ring7) ring7Touched = true;
			if(currentRing == ring8) ring8Touched = true;
			if(currentRing == ring9) ring9Touched = true;
			if(currentRing == ring10) ring10Touched = true;
			...
			if(currentRing == ring6 && ring6Touched) {
				ring6.setY(ring6.getY() - 20);
				ring = null;
				desableTouches();
				ring6Touched = false;
			}
			if(currentRing == ring7 && ring7Touched) {
				ring7.setY(ring7.getY() - 20);
				ring = null;
				desableTouches();
				ring7Touched = false;
			}
			if(currentRing == ring8 && ring8Touched) {
				ring8.setY(ring8.getY() - 20);
				ring = null;
				desableTouches();
				ring8Touched = false;
			}
			if(currentRing == ring9 && ring9Touched) {
				ring9.setY(ring9.getY() - 20);
				ring = null;
				desableTouches();
				ring9Touched = false;
			}
			if(currentRing == ring10 && ring10Touched) {
				ring10.setY(ring10.getY() - 20);
				ring = null;
				desableTouches();
				ring10Touched = false;
			}
	...
	private void changeRingPosition(Sprite tower, Ring ring) {
	    ...
	    if(stack3.size() == 10) {
	    	...
	    	pauseGame();
	    	levelCompleteWindow.display(8, contador, getMoves(), necessaryMoves);
	    }
	}
	...
	private void checkMoves() {
		...
		if(contador < dbHelper.getRecord(8)) {
			dbHelper.setRecord(8, contador);
		}
	}
	
	private int getMoves() {
		DBHelper dbHelper = new DBHelper(activity);
		return dbHelper.getRecord(8);
	}
	...
	private void pauseGame() {
		...
		unregisterTouchArea(ring6);
		unregisterTouchArea(ring7);
		unregisterTouchArea(ring8);
		unregisterTouchArea(ring9);
		unregisterTouchArea(ring10);
	}
}
